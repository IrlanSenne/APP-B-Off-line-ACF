...
import {SafeAreaView, StyleSheet, Text, TouchableOpacity, ScrollView, View,Alert,Share  } from "react-native";
...
componentDidMount = async() =>{
  ...
  requestLocal(){
    ...
    const numeroVers = this.state.biblia[livro][cap].v.length
 
      for(let v = 0; v< numeroVers ;v++){
        let versiculo=this.state.biblia[livro][cap].v[v].t
...
if(normal.indexOf(false) > -1){
  ...
  render(){
    ...
    return (
      ...
      <Header nomeLivro={this.state.livro} cap={this.state.cap + 1}  multiVersao={()=>this.multiVersao()}/>
        <View style={{flex:1, padding:1}}>
            ...
            <AdMobBanner
                        ...
                        onDidFailToReceiveAdWithError={(error)=>console.log('BANNER ERROR: ' , error)} />    
        ...
        {   
            this.state.contador > 0 ?
                      ...
                      this.state.cores.map((cor)=>{
                        ...
                        <Text><Ionicons name="share-social-outline" size={24} color="white" /></Text>
                        </TouchableOpacity>    
                        <TouchableOpacity  onPress={()=> this.copyToClipboard()} style={{width:27,height:27,margin:5,alignItems:'center',justifyContent:'center'}}>
                        <Text><Ionicons name="copy-outline" size={24} color="white" /></Text>
                        </TouchableOpacity>  
                      </View>  
              : null 
            }   
